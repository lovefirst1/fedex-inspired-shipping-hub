...
import { Package, MapPin, Calendar, CheckCircle2, Clock, AlertTriangle, Search, Truck, PackageCheck, RefreshCw, DollarSign, Scale, Timer, Plane, Warehouse } from "lucide-react";
...
const Tracking = () => {
  ...
  const fetchShipment = useCallback(async (code: string, showLoading = true) => {
    ...
    try {
      const {
        data: shipmentData,
        error: shipmentError
      } = await supabase.from("shipments").select("*").eq("tracking_code", code).maybeSingle();
      ...
      const {
        data: timelineData,
        error: timelineError
      } = await supabase.from("shipment_timeline").select("*").eq("shipment_id", shipmentData.id).order("timestamp", {
        ascending: false
      });
  ...
  useEffect(() => {
    ...
    const shipmentsChannel = supabase.channel('shipment-updates').on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'shipments',
    ...
    const timelineChannel = supabase.channel('timeline-updates').on('postgres_changes', {
      event: 'INSERT',
      schema: 'public',
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          ...
          <Card className="p-4 md:p-6 mb-6 md:mb-8 shadow-lg border-t-4 border-t-primary">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <Input type="text" placeholder="Enter tracking number" value={trackingCode} onChange={e => setTrackingCode(e.target.value.toUpperCase())} onKeyPress={e => e.key === "Enter" && handleSearch()} className="flex-1 h-12 text-base" />
              <Button onClick={handleSearch} size="lg" disabled={loading} className="bg-accent hover:bg-accent/90 h-12 px-8">
                <Search className="mr-2 h-5 w-5" />
                {loading ? "Searching..." : "Track"}
              </Button>
            </div>
            {lastUpdated && shipment && <div className="flex items-center gap-2 mt-3 text-xs text-muted-foreground">
          ...
          {/* Shipment Details */}
          {shipment && <div className="space-y-4 md:space-y-6 animate-fade-in">
              {/* Status Card */}
              <Card className="p-4 md:p-6 shadow-lg">
                ...
                <div className="mb-6 pb-6 border-b overflow-x-auto">
                  <ShipmentProgressTracker status={shipment.status} heldByCustoms={shipment.held_by_customs} className="min-w-[600px]" />
                </div>

                {shipment.held_by_customs && <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
                ...
                {shipment.package_description && <div className="mt-6 pt-6 border-t border-border">
                ...
                {shipment.notes && <div className="mt-4 p-3 bg-muted rounded-lg">
              ...
              {/* Timeline */}
              {timeline.length > 0 && <Card className="p-4 md:p-6 shadow-lg">
                  <h3 className="text-xl font-bold text-foreground mb-6 flex items-center gap-2">
                    <Clock className="h-5 w-5 text-primary" />
                    Shipment Timeline
                  </h3>
                  <div className="space-y-0">
                    {timeline.map((event, index) => <div key={event.id} className="flex space-x-4">
                        <div className="flex flex-col items-center">
                          <div className={`flex-shrink-0 p-2 rounded-full ${index === 0 ? 'bg-primary/10' : 'bg-muted'}`}>
                            {getStatusIcon(event.status)}
                          </div>
                          {index < timeline.length - 1 && <div className="w-0.5 h-full min-h-[40px] bg-border" />}
                        </div>
                        <div className="flex-1 pb-6">
                          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-1">
                            <div>
                              ...
                              {event.location && <p className="text-sm text-muted-foreground flex items-center mt-1">
